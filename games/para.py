...
def load_all_images():
    ...
    for i in range(1, 16):
        fname = None
        if os.path.exists(f"img/{i}.png"):
            fname = f"img/{i}.png"
        elif os.path.exists(f"img/{i}.jpg"):
            fname = f"img/{i}.jpg"
        if fname:
            try:
                images.append(pygame.image.load(fname))
            except:
                pass
...
class Garland:
    ...
    def draw(self, screen, time_now):
        ...
        for b in self.bulbs:
            ...
            final_col = (min(255, int(r * brightness + 50)), min(255, int(g * brightness + 50)),

...
class GiftBox:
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(
            screen,
            self.ribbon,
            (self.rect.centerx - rw // 2, self.rect.y, rw, self.rect.h),
        )
        pygame.draw.rect(
            screen, self.ribbon, (self.rect.x, self.rect.centery - 5, self.rect.w, rw)
        )
        pygame.draw.ellipse(
            screen, self.ribbon, (self.rect.centerx - 8, self.rect.y - 6, 8, 8)
        )
        pygame.draw.ellipse(
            screen, self.ribbon, (self.rect.centerx, self.rect.y - 6, 8, 8)
        )
...
class Snowman:
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(screen, (30, 20, 20), (x + 7, y - 80), 3)
        pygame.draw.polygon(
            screen, (230, 100, 20), [(x, y - 75), (x, y - 70), (x + 15, y - 72)]
        )
        ...
        pygame.draw.line(screen, (60, 40, 20), (x + 55, y - 50), (x + 65, y - 65), 2)
        pygame.draw.rect(
            screen, COLORS["caramel_text"], (x - 22, y - 60, 44, 10), border_radius=3
        )
        pygame.draw.rect(
            screen, COLORS["caramel_text"], (x + 5, y - 55, 12, 30), border_radius=3
        )
        hat_color = (50, 20, 20)
        ...
        for i in range(400):
            ...
            for _ in range(5):
                ...
                if random.random() < 0.15:
                    sl = l * 0.4
                    self.snow.append(
                        ((nx - math.cos(a) * sl, ny - math.sin(a) * sl), (nx, ny))
                    )
            if random.random() < 0.03:
                self.orn.append(
                    (
                        (bx + math.sin(ang) * 10, by + math.cos(ang) * 15 + 10),
                        random.choice(ORNAMENT_COLORS),
                    )
                )

    def draw(self, s):
        for c, st, en in self.layers:
            pygame.draw.aaline(s, c, st, en)
        for st, en in self.snow:
            pygame.draw.line(s, PINE_PALETTE["snow"], st, en, 2)
        for p, c in self.orn:
...
class Garland:
    def __init__(self):
        self.b = [
            {
                "p": (int(WIDTH / 20 * i), int(-15 + math.sin(i / 20 * math.pi) * 70)),
                "c": LIGHT_COLORS[i % 4],
                "on": True,
            }
            for i in range(21)
    ...
    def draw(self, s):
        if len(self.b) > 1:
            pygame.draw.lines(s, (30, 30, 30), False, [x["p"] for x in self.b], 3)
        for x in self.b:
            ...
            if x["on"]:
                ...
                pygame.draw.circle(
                    s, tuple(c // 4 for c in x["c"]), (x["p"][0], x["p"][1] + 5), 6
                )
...
class Button:
    ...
    def draw(self, s):
        c = (
            COLORS["btn_click"]
            if self.st == "clicked"
            else (COLORS["btn_hover"] if self.st == "hover" else COLORS["btn_normal"])
        )
...
class Game_2048:
    def __init__(self):
        ...
        self.fonts_tile = {
            "l": get_font(f, 70, True),
            "m": get_font(f, 55, True),
            "s": get_font(f, 40, True),
        }

        self.btn_new = Button((50, 420, 220, 60), "NEW GAME", self.fb)
        self.btn_try = Button(
            (WIDTH // 2 - 120, HEIGHT // 2 + 80, 240, 60), "TRY AGAIN", self.fb
        )
    ...
    def add_tile(self, count=1):
        for _ in range(count):
            empty = [
                (r, c) for r in range(4) for c in range(4) if self.matrix[r][c] == 0
            ]
            if empty:
                        ...
                        moves.append(
                            {
                                ...
                                "start": (r, c),
                                "end": (nr, nc),
                                "pixel_start": self.get_pos(r, c),
                                "pixel_end": self.get_pos(nr, nc),
                                "is_merge": True,
                            }
                        )
                        break
    ...
    def get_pos(self, r, c):
        return (
            BOARD_START_X + GRID_PADDING + c * (TILE_SIZE + GRID_PADDING),
    ...
    def update(self):
        for s in self.snow:
            s[1] += s[3]
            if s[1] > HEIGHT:
                s[1], s[0] = random.randint(-50, -10), random.randint(0, WIDTH)
        self.decor[0].update()
        if self.anim_state == "SLIDE":
            ...
            if self.pop_tile:
                ...
                if self.pop_tile["scale"] >= 1.0:
                    self.pop_tile["scale"] = 1.0
                    self.anim_state = "IDLE"
                    self.pop_tile = None
    ...
    def draw(self):
        ...
        self.screen.blit(
            self.fst.render("Exclusive Edition", True, (170, 130, 130)), (55, 245)
        )
        ...
        self.screen.blit(
            self.fsl.render("SCORE", True, COLORS["text_label"]),
            (score_rect.centerx - 28, score_rect.y + 12),
        )
        sc = self.fsv.render(str(self.score), True, COLORS["text_val"])
        self.screen.blit(
            sc, (score_rect.centerx - sc.get_width() // 2, score_rect.y + 35)
        )

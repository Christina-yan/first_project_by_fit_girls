            ...
            if game_active and not paused:
                ...
                if lose_image:
                    ...
                    if current_scale > 0.1:
                        ...
                        final_img = pygame.transform.smoothscale(
                            lose_image, (final_w, final_h)
                        )
                        ...
                        screen.blit(
                            best_text,
                            (
                                panel_x + panel_w - 20 - best_text.get_width(),
                                panel_y + 15,
                            ),
                        )
                        # Разделитель
                        ...
                        retry_hint = font_small.render(
                            "SPACE - Retry", True, (200, 200, 200)
                        )
                        menu_hint = font_small.render(
                            "ESC - Menu", True, (150, 150, 150)
                        )
                        screen.blit(
                            retry_hint,
                            (WIDTH // 2 - retry_hint.get_width() // 2, panel_y + 55),
                        )
                        screen.blit(
                            menu_hint,
                            (WIDTH // 2 - menu_hint.get_width() // 2, panel_y + 75),
                        )
                else:
                    ...
                    screen.blit(
                        l1, (WIDTH // 2 - l1.get_width() // 2, HEIGHT // 2 - 50)
                    )
                    screen.blit(
                        l2, (WIDTH // 2 - l2.get_width() // 2, HEIGHT // 2 + 10)
                    )
